...
var app = new Vue({
  ...
  computed: {
    ...
    hasSubnews: function hasSubnews() {
      return !!this.subnews;
    },
  ...
  methods: {
    ...
    generateScreenshot: function generateScreenshot() {
      ...
      var genericImage = document.querySelector('.generic-image');
      this.triggerDownload.classList.add('opacity-50');
      axios.post('/compose', data).then(function (response) {
        if (response.data.success === true) {
          // set the image
          genericImage.src = response.data.image_url; // let Vue know about the new image

          vm.generatedImage = response.data.image_url; // set the download button href

          vm.downloadAction.href = vm.baseHref + '/download/?f=' + response.data.image_url;
          vm.downloadAction.click();
          vm.triggerDownload.classList.remove('opacity-50'); // hide the sidebar
  ...
  watch: {
    ...
    subnews: function subnews() {
      this.memeChanged = true;
    },
    isLive: function isLive() {
      this.memeChanged = true;
    },
    isNewsAlert: function isNewsAlert() {
      this.memeChanged = true;
    },
    isBreakingNews: function isBreakingNews() {
      this.memeChanged = true;
    },
    time: function time() {
      this.memeChanged = true;
